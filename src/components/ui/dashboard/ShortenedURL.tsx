...
export default function Dashboard_ShortenedURL(
    {data}: ShortenedURLProps
) {
    const handleCopy = useCallback((event: React.MouseEvent<HTMLButtonElement>) => {
        void copyToClipboard(getFullShortenedURL(data, true))
            .then(() => {
    ...
    return (
        <Paper sx={{
            ...
            <Box>
                <Avatar src={getFaviconFromURL(data.originalURL)} sx={{
                    bgcolor: "white",
                    p: 1
                    ...
                    <Box>
                        <Typography variant={'caption'} sx={{
                            ...
                            <Box sx={{
                                mt: .7,
                                ml: .5
                            }}>{data.createdAt.toLocaleDateString()}</Box>
                        </Typography>
                    </Box>
                </Box>
            </Box>
